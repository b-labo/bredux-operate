...
function reducer (state = {}, action) {
  let { type: actionType, payload, meta = {} } = action
  let [scopeName, operationName] = actionType.split(':')
  if (!operationName) {
    return state
  }
  let scopeState = state[ scopeName ]
  return assign({}, state, {
    [scopeName]: reducer.operate(scopeState, { type: operationName, payload, meta })
  })
}

assign(reducer, {
  operate (state = {}, { type, payload, meta }) {
    ...
    switch (String(type).toLowerCase().trim()) {
      ...
      case POP: {
        let fieldState = or(state[ fieldName ], [])
        return fieldState.slice(0, state.length - 1)
      }
      case SHIFT: {
        let fieldState = or(state[ fieldName ], [])
        return fieldState.slice(1)
      }
      case POP_ALL: {
        return []
      }
      case REJECT: {
        let fieldState = or(state[ fieldName ], [])
        return fieldState.filter((state) => state !== payload)
      }
      // --------------------
      // Boolean operations
      // --------------------
